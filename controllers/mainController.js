...
const save_model = require("../models/itemSave");
const offer_model = require("../models/itemOffer");
...
exports.wishitem = (req, res, next) => {
  console.log("Hey!")
  let id = req.params.id;
  save_model
    .findById(id)
    .then((item) => {
      res.render("./item/savepage", { item });
    })
    .catch((err) => {
      next(err);
    });
...
exports.wishdelete = (req, res, next) => {
  console.log("in wish deete");
  let id = req.params.id;
  save_model
    .findByIdAndDelete(id, { useFindAndModify: false })
    .then((item) => {
      req.flash("success", "Item has been deleted from saved trades!!");
